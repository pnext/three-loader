...
onmessage = function (event) {
  ...
  */
  ...
  const CORE_DATA_SIZE = 28;
  const HARMONICS_DATA_SIZE = 45;
  const bytesOffsetScale = bytesPerPointPosition;
  const bytesOffsetOpacity = bytesOffsetScale + bytesPerPointScale;
  const bytesOffsetColor = bytesOffsetOpacity + bytesPerPointOpacity;
  const bytesOffsetOrientation = bytesOffsetColor + bytesPerPointColor;
  ...
  for (let j = 0; j < numPoints; j++) {
    //Positions
    const pointOffset = j * CORE_DATA_SIZE;
    ...
    const colorOffset = j * CORE_DATA_SIZE + bytesOffsetColor;
    const opacityOffset = j * CORE_DATA_SIZE + bytesOffsetOpacity;
    ...
    //Scale
    const scaleOffset = j * CORE_DATA_SIZE + bytesOffsetScale;
    ...
    //Orientation
    const rotationOffset = j * CORE_DATA_SIZE + bytesOffsetOrientation;

    //Decoding based on: https://github.com/Pix4D/pix4d-gsplat/blob/master/docs/GS_OPF_glTF_requirements.md
    encoded = view.getUint32(rotationOffset, true);

    //Read the first two bits to get the largest component
    const largestComponent = (encoded >> 30) & 3;
    const ca = decodeComponent(20);
    const cb = decodeComponent(10);
    const cc = decodeComponent(0);
    const cd = Math.sqrt(1 - ca * ca - cb * cb - cc * cc);
  ...
  if (harmonicsEnabled) {
    for (let j = 0; j < numPoints; j++) {
      for (let z = 0; z < 15; z++) {
        //Size of the core buffer + size of total harmonics + offset of harmonics.
        const harmonicsOffset =
          numPoints * CORE_DATA_SIZE + j * HARMONICS_DATA_SIZE + z * bytesPerPointHarmonics;
        ...
        harmonics[45 * j + z * 3 + 0] = r;
        harmonics[45 * j + z * 3 + 1] = g;
        harmonics[45 * j + z * 3 + 2] = b;
      }
    }
  }
